...
describe("v0.2 release checklist", () => {
  it("documents release tag and install-pin verification without authorizing publication", () => {
    ...
    expect(checklist).toContain("git tag v0.2.0");
    expect(checklist).toContain("git push origin v0.2.0");
    expect(checklist).toContain("git ls-remote --tags origin refs/tags/v0.2.0");
    expect(checklist).toContain("pi install -l git:github.com/DanielCarva1/pi-bmad-runtime@v0.2.0");
