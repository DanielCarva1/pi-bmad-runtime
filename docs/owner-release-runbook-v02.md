...
Use this runbook only after the Owner explicitly decides to publish `pi-bmad-runtime v0.2.2` to GitHub. Until then, every command in the readiness path stays local and read-only.
...
```bash
git commit -m "release: pi-bmad-runtime v0.2.2"
git tag v0.2.2
...
- Stop if the remote tag verification does not show `refs/tags/v0.2.2`.
